...
def election_summary(election_id: int):
    ...
    party_totals = fetchall(
        """
        SELECT p.id, p.name, p.short_name, p.color_hex, p.ballot_number, COALESCE(p.is_minority, false) AS is_minority, COALESCE(SUM(mr.votes), 0) AS votes
        FROM parties p
        LEFT JOIN municipality_results mr ON mr.party_id = p.id AND mr.election_id = :eid
        WHERE p.election_id = :eid
        GROUP BY p.id, p.name, p.short_name, p.color_hex, p.ballot_number, p.is_minority
        ORDER BY votes DESC
        """,
        {"eid": election_id},
    )
...
def municipalities_results(election_id: int):
    ...
    party_rows = fetchall(
        """
        SELECT mr.municipality_id, p.id, p.name, p.short_name, p.color_hex, p.ballot_number, COALESCE(p.is_minority, false) AS is_minority, mr.votes
...
def regions_results(election_id: int):
    ...
    party_rows = fetchall(
        """
        SELECT m.region AS region, p.id, p.name, p.short_name, p.color_hex, p.ballot_number, COALESCE(p.is_minority, false) AS is_minority, SUM(mr.votes) AS votes
        FROM municipality_results mr
        JOIN parties p ON p.id = mr.party_id
        JOIN municipalities m ON m.id = mr.municipality_id
        WHERE mr.election_id = :eid
        GROUP BY m.region, p.id, p.name, p.short_name, p.color_hex, p.ballot_number, p.is_minority
...
def _station_detail(election_id: int, station_id: int):
    ...
    cached = fetchall(
        """
        SELECT p.name, p.short_name, p.color_hex, p.ballot_number, COALESCE(p.is_minority, false) AS is_minority, r.votes, r.is_processed
        FROM results r
